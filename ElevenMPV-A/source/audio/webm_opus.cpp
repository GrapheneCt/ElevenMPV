...
SceInt32 audio::WebmOpusDecoder::NeRead(ScePVoid buffer, SceSize length, ScePVoid userdata)
{
	// success = 1
	// eof = 0
	// error = -1
...
SceInt32 audio::WebmOpusDecoder::NeSeek(SceInt64 offset, SceInt32 whence, ScePVoid userdata)
{
	SceAvPlayerFileReplacement *fio = (SceAvPlayerFileReplacement *)userdata;
...
SceInt64 audio::WebmOpusDecoder::NeTell(ScePVoid userdata)
{
	return s_currPos;
}

audio::WebmOpusDecoder::WebmOpusDecoder(const char *path, SceBool isSwDecoderUsed) : GenericDecoder::GenericDecoder(path, isSwDecoderUsed)
{
	SceInt32 ret = 0;
	SceUInt32 lures = 0;
	SceUInt64 llures = 0;
	...
	LOCALMediaInit(&nmHandle, &fio, SCE_NULL, bufMemSize, 0, 0);
...
SceUInt32 audio::WebmOpusDecoder::GetSampleRate()
{
	return sampleRate;
}

SceUInt8 audio::WebmOpusDecoder::GetChannels()
{
	return channelNum;
}

SceVoid audio::WebmOpusDecoder::Decode(ScePVoid stream, SceUInt32 length, ScePVoid userdata)
{
	SceInt32 ret = 0;
	SceUInt32 uret = 0;
...
SceUInt64 audio::WebmOpusDecoder::Seek(SceFloat32 percent)
{
...
audio::WebmOpusDecoder::~WebmOpusDecoder()
{
	isPlaying = SCE_TRUE;
	isPaused = SCE_FALSE;
